...
const MODEL_VERSION = "setup-alerts-sniper70-v1";
...
function buildSniperDecision(scan) {
  ...
  if (scan.pair === "BTCUSD" && scan.volatility > 0.04) {
    blockers.push("BTC volatility extreme");
  }

  if (scan.pair === "XAUUSD" && scan.volatility > 0.025) {
    blockers.push("Gold volatility extreme");
  }

  const sniperScore = clamp(
    scan.ultraScore * 0.26 +
      scan.entryQualityScore * 0.24 +
      scan.mtfScore * 0.18 +
      scan.riskScore * 0.10 +
      scan.paperScore * 0.14 +
      (100 - scan.exitPressureScore) * 0.08,
    1,
    99
  );
  ...
  return {
    ...
    reason: blockers.length
      ? blockers.join(" · ")
      : "SNIPER 70 setup validated: score, entry, MTF, risk and exit pressure accepted."
  };
}
...
function computeUltraScore(data) {
  let score = clamp(
    Number(data.localScore || 50) * 0.26 +
      Number(data.trendScore || 50) * 0.16 +
      Number(data.timingScore || 50) * 0.14 +
      Number(data.executionScore || 50) * 0.16 +
      Number(data.smartMoneyScore || 50) * 0.10 +
      Number(data.riskScore || 50) * 0.08 +
      Number(data.sessionScore || 50) * 0.10,
    1,
    99
  );
  ...
  if (data.pair === "BTCUSD") {
    ...
    if (data.volatility >= 0.003 && data.volatility <= 0.02) score += 4;
    if (data.volatility > 0.04) score -= 12;
  }

  if (data.pair === "XAUUSD") {
    if (data.timeframe === "H1" || data.timeframe === "H4") score += 2;
    if (data.volatility > 0.025) score -= 8;
  }
...
function computeEntryQualityScore(data) {
  ...
  score += (Number(data.ultraScore || 0) - 70) * 0.22;
  score += (Number(data.trendScore || 50) - 50) * 0.16;
  score += (Number(data.timingScore || 50) - 50) * 0.17;
  score += (Number(data.executionScore || 50) - 50) * 0.18;
  score += (Number(data.smartMoneyScore || 50) - 50) * 0.10;
  score += (Number(data.riskScore || 50) - 50) * 0.07;

  const trigger = computeCandleTriggerScore(data.candles, data.direction);
  score += trigger.score;
  reasons.push(...trigger.reasons);

  const extension = computeLateEntryPenalty(data);
...
function computeExitPressureScore(data) {
  ...
  score += weakness(data.trendScore, 50) * 0.20;
  score += weakness(data.timingScore, 48) * 0.18;
  score += weakness(data.executionScore, 48) * 0.18;
  score += weakness(data.smartMoneyScore, 48) * 0.12;
  score += weakness(data.riskScore, 44) * 0.12;
  ...
  if (data.ultraScore >= 82 && data.executionScore >= 62) {
    score -= 8;
  }
...
function computePaperScore(data) {
  return clamp(
    Number(data.ultraScore || 0) * 0.30 +
      ...
      Number(data.executionScore || 50) * 0.14 +
      Number(data.smartMoneyScore || 50) * 0.10 +
      Number(data.riskScore || 50) * 0.08 +
      Number(data.sessionScore || 50) * 0.05 +
      (100 - Number(data.exitPressureScore || 50)) * 0.08,
    1,
    99
  );
}
...
function computeLateEntryPenalty(data) {
  ...
  const ema20Value = ema(closes, 20);
  const atrValue = Number(data.atr14 || atr(highs, lows, closes, 14));
  ...
  const maxDistance =
    data.pair === "BTCUSD" ? atrValue * 2.8 :
    data.pair === "XAUUSD" ? atrValue * 2.4 :
    atrValue * 2.1;
...
function isLateEntry(data) {
  const late = computeLateEntryPenalty(data);
